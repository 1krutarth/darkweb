...
s3 = FOREACH dns GENERATE sitename, (int)httpcode AS (httpcode:int), GetDay(timestamp) as (day:int), GetMonth(timestamp) as (month:int), GetYear( timestamp ) as (year:int);
